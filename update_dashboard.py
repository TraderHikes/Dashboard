...
def safe_float(val, default=None):
    """Safely extract a scalar float from a yfinance result."""
    try:
        if val is None:
            return default
        if isinstance(val, pd.Series):
            val = val.dropna()
            return float(val.iloc[-1]) if len(val) else default
        return float(val)
    except Exception:
        return default
...
def fetch_nifty500_symbols():
    ...
    try:
        ...
        r = requests.get(url, headers={"User-Agent":"Mozilla/5.0","Accept":"text/csv"}, timeout=20)
        r.raise_for_status()
...
# ════════════════════════════════════════════════════════════
# INDEX LEVELS
# Schema: ONE wide row per date.
# Columns: snapshot_date, nifty50, nifty50_prev,
#          sensex, sensex_prev, nifty500, nifty500_prev,
#          nifty_midcap, nifty_midcap_prev,
#          nifty_it, nifty_it_prev, nifty_bank, nifty_bank_prev,
#          nifty_pharma, nifty_pharma_prev, nifty_auto, nifty_auto_prev,
#          nifty_psubank, nifty_psubank_prev, nifty_metal, nifty_metal_prev,
#          nifty_realty, nifty_realty_prev, nifty_fmcg, nifty_fmcg_prev
# ════════════════════════════════════════════════════════════
INDEX_MAP = {
    "nifty50":      "^NSEI",
    "sensex":       "^BSESN",
...
def fetch_index_levels():
    print("📊 Fetching index levels (wide row)...")
    ...
    for col_key, ticker in INDEX_MAP.items():
        try:
            ...
            close = data["Close"].dropna()   # always a Series
            if len(close) < 2:
            ...
            if col_key == "nifty500":
                n500_val = current
            print(f"   ✓ {col_key}: {current:,.2f} (prev {prev:,.2f})")
        except Exception as e:
            print(f"   ⚠️  {col_key} ({ticker}): {e}")

    if len(row) > 1:
        sb.table("index_levels").upsert(row, on_conflict="snapshot_date").execute()
        print(f"   ✓ Saved index_levels row for {TODAY}\n")
    return row, n500_val


# ════════════════════════════════════════════════════════════
# PORTFOLIO SNAPSHOT
# Schema columns: snapshot_date, portfolio_value,
#                 total_capital, cash_available,
#                 nifty500_level, cumulative_return_pct
# ════════════════════════════════════════════════════════════
def save_portfolio_snapshot(cmp_map, n500_val):
    ...
    try:
        ...
        print(f"   ✓ Snapshot: portfolio ₹{port_val:,.0f} | cash ₹{cash_avail:,.0f}\n")
    except Exception as e:
...
# ════════════════════════════════════════════════════════════
# MARKET BREADTH
# Schema columns: snapshot_date, total_stocks,
...
#   nifty50_close, regime_score, regime_label, ad_ratio
# ════════════════════════════════════════════════════════════
def calculate_market_breadth(symbols, nifty50_close=None):
    ...
    for i in range(0, len(symbols), BATCH):
        ...
        try:
            ...
            for sym in batch:
                try:
                    close = (raw[sym]["Close"] if len(batch)>1 else raw["Close"]).dropna()
                    ...
                    if cur > float(compute_ema(close, 21).iloc[-1]):  a21  += 1
                    if cur > float(compute_ema(close, 50).iloc[-1]):  a50  += 1
                    if cur > float(compute_ema(close, 200).iloc[-1]): a200 += 1
    ...
    print(f"   ✓ Breadth: {valid} stocks | 21:{p21}% 50:{p50}% 200:{p200}% | {label}\n")
...
def fetch_fii_dii():
    ...
    try:
        ...
        sess.get("https://www.nseindia.com",
                 headers={"User-Agent":"Mozilla/5.0","Accept":"*/*"}, timeout=10)
        ...
        sb.table("fii_dii_activity").upsert({
            "activity_date": TODAY, "fii_cash_net": fii_net,
            "dii_cash_net": dii_net, "source": "NSE"
...
def fetch_candles_for_open_trades():
    ...
    try:
        ...
        for sym in list({t["symbol"].strip().upper() for t in trades}):
            try:
                hist = yf.Ticker(f"{sym}.NS").history(period="180d",
...
# ════════════════════════════════════════════════════════════
# SECTORAL BREADTH
# Schema: sector_breadth table (created by sector_schema.sql)
# ════════════════════════════════════════════════════════════
...
def fetch_sector_breadth():
    ...
    try:
        ...
        # Nifty 500 for RS baseline
        n500 = yf.download("^CRSLDX", period="6mo", progress=False,
        ...
        for sk, meta in sector_map.items():
            ...
            try:
                ...
                for sym in syms:
                    try:
                        cl = (raw[sym]["Close"] if len(syms)>1 else raw["Close"]).dropna()
                ...
                if tk:
                    try:
                        idx = yf.download(tk, period="6mo", progress=False,
                                          auto_adjust=True)["Close"].dropna()
                        if len(idx) >= 2:
        ...
        if results:
            ...
            print(f"   ✓ Saved {len(results)} sectors for {TODAY}\n")
    except Exception as e:
...
if __name__ == "__main__":
    try:
        if RUN_MODE == "intraday":
            ...
            idx_row, n500_val = fetch_index_levels()
            nifty50_close    = idx_row.get("nifty50")
            save_portfolio_snapshot(cmp_map, n500_val)
            calculate_market_breadth(symbols, nifty50_close)
            fetch_candles_for_open_trades()
            fetch_fii_dii()
            fetch_sector_breadth()

        print(f"\n{'='*56}\n✅  {RUN_MODE.upper()} complete — {NOW_IST.strftime('%d %b %Y · %I:%M %p IST')}\n{'='*56}\n")
